...
class EmployeesController extends Controller
{
		...
		if(Module::hasAccess("Employees", "create")) {
			...
			if(env('MAIL_USERNAME') != null && env('MAIL_USERNAME') != "null" && env('MAIL_USERNAME') != "") {
				...
				Mail::send('emails.send_login_cred', ['user' => $user, 'password' => $password], function ($m) use ($user) {
					$m->from('lyngochung.dev@gmail.com', 'Bsstech Admin');
					$m->to($user->email, $user->name)->subject('Bsstech Admin - Your Login Credentials');
				});
			} else {
	...
	{
		$rolesUser = Auth::user()->roles->first()->name;
		$values = DB::table('employees')->select($this->listing_cols)->whereNull('deleted_at')->get();
		foreach($values as $k=> $v){
			$user =  $user = User::where('context_id', '=', $v->id)->firstOrFail();
			if($rolesUser != "SUPER_ADMIN" && $user->roles->first()->name == "SUPER_ADMIN"){
				unset($values[$k]);
			}
		}
