...
class CodeGenerator
{
	...
	**/
    ...
    public static function generateConfig($module, $icon)
    {
        ...
        $tableP = str_plural(strtolower($module),1);
        $tableS = str_singular(strtolower($module));
        $config->dbTableName = $tableP;
        $config->fa_icon = $icon;
        $config->moduleName = ucfirst(str_plural($module,1));
		$config->moduleName2 = str_replace('_', ' ', ucfirst(str_plural($module,1)));
        $config->controllerName = ucfirst(str_plural($module,1))."Controller";
